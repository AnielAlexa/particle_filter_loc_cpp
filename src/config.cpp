...
FullConfig load_config(const std::string& yaml_path) {
    ...
    if (auto p = root["particle_filter"]) {
        ...
        pf.obs_min_inliers_ref_alt = get(p, "obs_min_inliers_ref_alt", pf.obs_min_inliers_ref_alt);
        pf.obs_min_inliers_max = get(p, "obs_min_inliers_max", pf.obs_min_inliers_max);
        pf.inlier_tau = get(p, "inlier_tau", pf.inlier_tau);
        pf.init_lat = get(p, "init_lat", pf.init_lat);
        pf.init_lon = get(p, "init_lon", pf.init_lon);
        pf.init_yaw = get(p, "init_yaw", pf.init_yaw);
