...
public:
    VpiPreprocessor(int coarse_size = 256, int fine_w = 512, int fine_h = 288);
    ~VpiPreprocessor() = default;

    VpiPreprocessor(const VpiPreprocessor&) = delete;
    VpiPreprocessor& operator=(const VpiPreprocessor&) = delete;

    int fine_width() const { return fine_w_; }
    int fine_height() const { return fine_h_; }
    ...
    // Prepare fine drone input: mono8 -> resize to fine_w x fine_h -> normalize to [0,1] -> [1,1,fh,fw] float
    void prepare_fine(const uint8_t* mono_data, int width, int height, float* output_ptr);

    // Prepare fine patch input: BGR -> gray -> resize to fine_w x fine_h -> [1,1,fh,fw] float
